...
export interface ScanResult { ip: string; latency_ms: number; download_mbps: number; upload_mbps: number; error: string; }
export type ScanStatus = 'idle' | 'scanning' | 'stopping';

export function CfScannerTab() {
    const { cfScannerSettings, updateCfScannerSettings, resetCfScannerSettings, scanResults, clearScanResults, scanStatus, setScanStatus } = useAppStore();
    ...
    const isScanning = scanStatus === 'scanning' || scanStatus === 'stopping';
    ...
    const handleLoadRanges = async () => {
        ...
        try {
            const response = await axios.get('/api/v1/scanner/cf/ranges');
    ...
    const handleStartScan = async (isResuming: boolean) => {
        ...
        setSearchTerm("");
        if (!isResuming) {
            toast.info("Clearing previous history and starting new scan...");
            try { await api.clearCfScanHistory(); clearScanResults(); } catch { toast.error("Could not clear previous history."); }
        } else {
            toast.info("Resuming scan with existing history...");
        }
        try {
    ...
    const handleStopScan = async () => {
        ...
        try { await api.stopCfScan(); toast.success("Scanner stopped successfully."); } catch { toast.error("Failed to send stop signal."); } finally { setScanStatus('idle'); }
    };
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-2 gap-4"><Button onClick={() => handleStartScan(false)} disabled={isScanning} size="lg">{isScanning ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Scanning...</> : <><Play className="mr-2 h-4 w-4" />New Scan</>}</Button><Button variant="secondary" onClick={() => handleStartScan(true)} disabled={isScanning || scanResults.length === 0} size="lg"><RefreshCcw className="mr-2 h-4 w-4" />Resume</Button></div>
                            <Button onClick={handleStopScan} variant="destructive" size={"lg"} disabled={scanStatus !== 'scanning'}>{scanStatus === 'stopping' ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Stopping...</> : <><StopCircle className="mr-2 h-4 w-4" />Stop Scan</>}</Button>
                            <Dialog><DialogTrigger asChild><Button variant="outline" disabled={isScanning}><Settings className="mr-2 h-4 w-4" /> Advanced</Button></DialogTrigger>
                                <DialogContent className="sm:max-w-[480px]"><DialogHeader><DialogTitle>Advanced Scan Settings</DialogTitle><DialogDescription>Configure randomization and proxy options.</DialogDescription></DialogHeader>
                                    <fieldset disabled={isScanning} className="grid gap-6 py-4">
            ...
            <div className="lg:col-span-3">
                <Card>
                    <CardHeader>
                        ...
                        {cfScannerSettings.doSpeedtest && (<div className="flex items-center space-x-2 pt-4"><Label className="flex items-center gap-2 cursor-pointer"><Checkbox checked={onlySpeedtestResults} onCheckedChange={(c) => setOnlySpeedtestResults(Boolean(c))} />Show only speed-tested IPs</Label></div>)}
                    </CardHeader>
                    <CardContent>
                        <div className="border rounded-md max-h-[600px] overflow-auto">
                            <Table><TableHeader className="sticky top-0 z-10 bg-muted/95 backdrop-blur-sm"><TableRow><TableHead className="w-[150px]">IP</TableHead><TableHead>Latency</TableHead><TableHead>Download</TableHead><TableHead>Upload</TableHead><TableHead className="text-right w-12 p-2"><Button variant="ghost" size="icon" onClick={clearScanResults} disabled={scanResults.length === 0}><Trash2 className="size-4" /></Button></TableHead></TableRow></TableHeader>
                                <TableBody ref={animationParent}>
                                    {filteredAndSortedResults.length > 0 ? (filteredAndSortedResults.map((result) => (<TableRow key={result.ip}><TableCell className="font-mono">{result.ip}</TableCell><TableCell><Badge variant="secondary">{`${result.latency_ms}ms`}</Badge></TableCell><TableCell>{result.download_mbps > 0 ? `${result.download_mbps.toFixed(2)} Mbps` : '-'}</TableCell><TableCell>{result.upload_mbps > 0 ? `${result.upload_mbps.toFixed(2)} Mbps` : '-'}</TableCell><TableCell></TableCell></TableRow>))) : (<TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">{isScanning ? "Scanning..." : (searchTerm ? "No results match your search." : "No results yet.")}</TableCell></TableRow>)}
                                </TableBody>
                            </Table>
                        </div>
                    </CardContent>
